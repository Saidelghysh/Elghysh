...
function wirePlayer(){
  ...
  // قائمة إذاعات احتياطية - لو الأولى فشلت يجرب التالية تلقائيًا
  const RADIO_STATIONS = [
    "https://backup.qurango.net/radio/mix",
    "https://stream.radiojar.com/0tpy1h0kxtzuv",
    "https://backup.qurango.net/radio/tarateel",
    "https://n02.rcs-rds.ro/aacplus/443/quran.aac", // احتياطي إضافي عام
  ];
  ...
  function tryRadioStation(i){
    ...
    document.getElementById("pt-title").textContent = "إذاعة القرآن الكريم";
...
function initVisitorCounter(){
  ...
  fetch(`https://countapi.mileshilliard.com/api/v1/hit/${KEY}`)
    ...
    .then(data=>{
      ...
      if(el) el.textContent = Number(data.value).toLocaleString("ar-EG");
    })
    .catch(()=>{
      const el = document.getElementById("visitor-count");
